...
// import { updateMember } from "@/app/actions/adminMemberManagement";
// import { useToast } from "@/app/(general)/context/ToastContext";

// // Updated to match your new simplified Prisma Schema
// export type Member = {
//   id: string;
//   membershipId: string;
//   fullName: string;
...
//   // Populate the form when a member is selected
//   useEffect(() => {
//     if (member) {
//       setFormData({
...
//       );
//       setSelectedFile(null); // Reset any previously selected files
//     }
//   }, [member]);
...
//       // Only append the file if the admin actually selected a new one
//       if (selectedFile) {
//         formDataPayload.append("profilePicture", selectedFile);
//       }

//       // 2. Send to Server Action
//       const result = await updateMember(member.id, formDataPayload);
...
//                 Edit{" "}
//                 <span className="text-[var(--color-secondary)]">Member</span>
//               </h3>
//             </div>
...
//                 <input
//                   type="text"
//                   value={`ID: ${member.membershipId}`}
//                   className="w-full bg-gray-50/50 font-body text-[0.95rem] text-gray-500 border-b border-gray-200 py-3 px-2 outline-none cursor-not-allowed"
//                   readOnly
//                   title="Membership ID cannot be changed"
//                 />
//               </div>
...
//               <div
//                 className={`relative w-full border-b border-gray-300 py-3 flex items-center justify-between transition-colors focus-within:border-[var(--color-secondary)] group ${isSubmitting ? "opacity-50" : "cursor-pointer"}`}
//               >
//                 <span
//                   className={`font-body text-[0.95rem] truncate pr-4 ${selectedFile || member.profilePicture ? "text-gray-900" : "text-gray-400"}`}
//                 >
//                   {fileName}
//                 </span>
...
interface AdminMemberInput {
  ...
  ward: string;
  whatsappNumber: string;
  verified: boolean; // <-- Added to state
}
  ...
  useEffect(() => {
    if (member) {
      setFormData({
        ...
        ward: member.ward || "",
        whatsappNumber: member.whatsappNumber || "",
        verified: member.verified || false, // <-- Load existing verification status
      });
